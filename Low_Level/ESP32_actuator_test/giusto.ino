...
void loop() {
  if (Serial.available() > 0) {
    ...
    if (input.startsWith("SPEED")) {
      ...
      if (!isMotorMoving()) {
        // If motor is not moving, set the new target angle immediately
        setTargetAngle(newTargetAngle);
      } else {
        // If motor is moving, store the new target angle for later execution
        targetAngle = newTargetAngle;
        Serial.print("New target angle stored: ");
  ...
  if (targetAngle != 0 && !isMotorMoving()) {
    moveToAngle(targetAngle);
    targetAngle = 0; // Reset the target angle after moving
  }
